...
def open_folder(add_date, modified_date, folder_name):
    global level
    return_value = f'{preset.NEW_LINE}{new_tab()}<DT><H3 ADD_DATE="{utils.date_to_epoch(add_date)}" LAST_MODIFIED="{utils.date_to_epoch(modified_date)}">{folder_name}</H3>{preset.NEW_LINE}{new_tab()}<DL><p>'
    level = level + 1
    return return_value
...
def close_folder():
    global level
    level = level - 1
    return f'{preset.NEW_LINE}{new_tab()}</DL><p>'


def add_url(url, add_date, url_title):
    return f'{preset.NEW_LINE}{new_tab()}<DT><A HREF="{str(url).strip()}" ADD_DATE="{utils.date_to_epoch(add_date)}">{str(url_title)}</A>'
